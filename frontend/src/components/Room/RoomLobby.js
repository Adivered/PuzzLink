...
import { switchToRoom, initializeRoomData } from "../../store/roomSlice"; 
...
const RoomLobby = () => {
  ...
  const handleStartGame = async () => {
    try {
      ...
      // Emit socket event immediately to start countdown UI
      emitSocketEvent('game_starting', { 
        roomId, 
        countdown: 3 // 3 second countdown
      });

      // Make API call in parallel (don't wait for it)
      const apiCall = fetch(`/api/rooms/${roomId}/start`, {
        method: 'POST',
        headers: {
      ...
      // After countdown, emit game started event
      setTimeout(async () => {
        try {
          // Wait for API call to complete now
          ...
          emitSocketEvent('game_started', { 
            roomId, 
            gameId: data.gameId 
          });
          
          // Show game started toast
          dispatch(addToast({
          ...
          console.error("Failed to complete game start:", error);
          ...
          // Emit game start failed event to reset UI
          emitSocketEvent('game_start_failed', { roomId });
        }
      }, 3000);

    } catch (error) {
      console.error("Failed to initiate game start:", error);
