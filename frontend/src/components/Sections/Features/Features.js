    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-20 left-10 w-32 h-32 opacity-10">
          ...
          } rotate-45 animate-spin`} style={{ animationDuration: '20s' }}></div>
        </div>
        
        <div className="absolute bottom-20 right-10 w-24 h-24 opacity-15">
          <div className={`w-full h-full ${
            theme === 'dark' ? 'bg-purple-400' : 'bg-purple-500'
          } rounded-full animate-pulse`} style={{ animationDuration: '4s' }}></div>
        </div>
        
        <div className="absolute top-1/2 right-1/4 w-20 h-20 opacity-10">
          <div className={`w-full h-full ${
            theme === 'dark' ? 'bg-pink-400' : 'bg-pink-500'
          } rotate-12 animate-bounce`} style={{ animationDuration: '6s' }}></div>
        </div>
