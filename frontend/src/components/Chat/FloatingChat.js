...
const FloatingChat = () => {
  ...
  const { isOpen, isMinimized, unreadCounts } = useSelector((state) => state.chat);
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 left-6 z-50 w-96 h-[500px] flex flex-col">
          <div
          ...
          >
            ...
            <div className="h-[436px] flex">
              {showConversations && (
                <div className={`w-1/3 border-r ${isDarkTheme ? 'border-gray-700' : 'border-gray-200'}`}>
                  <ConversationList onSelectConversation={() => setShowConversations(false)} />
                </div>
              )}
              <div className={`${showConversations ? 'w-2/3' : 'w-full'}`}>
                <ChatWindow />
              </div>
            </div>
          </div>
