...
const WhiteboardToolbar = ({
  ...
  return (
    <div className={`relative z-40 flex items-center justify-between p-4 border-b transition-colors duration-300 ${
      isDarkTheme 
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="relative">
          ...
          {showSizePicker && (
            <div className={`absolute top-12 left-0 z-50 p-4 rounded-xl shadow-2xl border backdrop-blur-sm transition-all duration-300 ${
              isDarkTheme 
                ? 'bg-gray-800/95 border-gray-700' 
                : 'bg-white/95 border-gray-200'
            }`}>
              <div className="space-y-3">
                <div className="text-xs font-medium text-center mb-3">
                ...
                <div className="grid grid-cols-5 gap-3">
                  {sizes.map((s) => (
                    <button
                      ...
                      className={`relative flex flex-col items-center justify-center p-3 rounded-lg transition-all duration-200 hover:scale-105 group ${
                        size === s
                          ? isDarkTheme
                    ...
                    >
                      {/* Visual size preview */}
                      <div 
                        className={`rounded-full mb-1 transition-colors duration-200 ${
                          size === s ? 'bg-white' : 'bg-current'
                        }`}
                        style={{ 
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (newSize >= 1 && newSize <= 100) {
                          handleSizeSelect(newSize);
                        }
                      }}
                      className={`w-16 px-2 py-1 text-xs rounded border transition-colors ${
                        isDarkTheme
                          ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-blue-500'
                          : 'bg-gray-50 border-gray-300 text-gray-800 focus:border-blue-500'
                      }`}
