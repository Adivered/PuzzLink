    ...
    >
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-5 auto-rows-min max-w-5xl mx-auto">
        {gameTypes.map((type) => (
          <div
            key={type.id}
            className={`cursor-pointer rounded-lg p-5 transition-all duration-300 hover:scale-105 border ${
              roomData.gameMode === type.id
                ...
                : isDarkTheme
                  ? "bg-gray-700 hover:bg-gray-600 shadow-sm border-gray-600"
                  : "bg-gray-50 hover:bg-gray-100 shadow-sm hover:shadow-md border-gray-200"
            }`}
