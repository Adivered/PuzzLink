...
const PuzzleGame = () => {
  ...
  const bankPieces = useMemo(() => {
    const pieces = displayPieces.filter(p => p.currentPosition === null || p.currentPosition === undefined);
    
    // Shuffle the pieces for a random order using a stable shuffle based on puzzle ID
    // This ensures the same order across renders but random initial placement
    const puzzleId = game?.puzzle?._id || 'default';
    const shuffled = [...pieces];
    
    // Simple seeded shuffle using puzzle ID as seed
    let seed = 0;
    for (let i = 0; i < puzzleId.length; i++) {
      seed += puzzleId.charCodeAt(i);
    }
  ...
  return (
    <div className={`h-[calc(100vh-4rem)] w-full mt-16 flex flex-col ${
